...
- [ ] Phase 10 verification: prove that a repeated search can render from cache first, stale cache can refresh safely, private local fields never appear in cloud payloads, and local-only mode still works with cloud cache disabled.
