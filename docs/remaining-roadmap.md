...
- [ ] Refresh worker and cron spike: design a small, retryable refresh path for popular/subscribed public queries using Vercel Cron only for lightweight scheduling, and a queue/worker layer if refresh jobs exceed serverless limits.
