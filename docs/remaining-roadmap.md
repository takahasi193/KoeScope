...
## Phase 10: Cloud Search Cache And Deployment Spike

- [x] Cloud cache boundary: define a stable public `queryKey` for DLsite search results from keyword/personId/aliases/scope/order/version, explicitly excluding account session, purchase history, watchlist, annotations, and other local-only data. Done 2026-05-16 on branch `optimize/phase-10-cloud-cache-boundary`; search payloads now expose a canonical public query key without local-only fields.
- [x] Search cache contract: add an interface-level design for cached search reads and refresh status so the UI can show cached results immediately, then continue background refresh without forcing a full re-search every time. Done 2026-05-16 on branch `optimize/phase-10-cloud-cache-boundary`; search payloads now expose `cache.read` and `cache.refresh` metadata while current execution remains local live search.
- [x] Storage adapter spike: evaluate Postgres/Neon/Supabase or Turso/libSQL for public search-result cache storage while keeping current local SQLite repositories as the local-first implementation. Done 2026-05-16 on branch `optimize/phase-10-cloud-cache-boundary`; added a local SQLite `public_search_cache` adapter, public-only payload sanitization, completion-time cache writes, and provider notes in `docs/phase-10-storage-adapter-spike.md`.
- [x] Vercel read-only prototype: deploy only the Next frontend and a small read-only cache API to Vercel; do not move Chrome Companion imports, DLsite cookies, account data, purchase snapshots, or local maintenance operations into Vercel. Done 2026-05-16 on branch `optimize/phase-10-vercel-readonly-prototype`; added `vercel.json`, a read-only public cache function, static public-cache smoke backend support, and deployment-boundary notes in `docs/phase-10-vercel-readonly-prototype.md`. Live Vercel deployment still requires project/token configuration outside this local run.
- [x] Object image cache spike: evaluate Vercel Blob, Cloudflare R2, or S3-compatible storage for public work/activity images, with remote DLsite image URLs preserved as fallback. Done 2026-05-16 on branch `optimize/phase-10-object-image-cache-spike`; added provider-neutral public object image keys, HTTPS object URL resolution, explicit remote URL fallback tests, and provider notes in `docs/phase-10-object-image-cache-spike.md`.
- [x] Refresh worker and cron spike: design a small, retryable refresh path for popular/subscribed public queries using Vercel Cron only for lightweight scheduling, and a queue/worker layer if refresh jobs exceed serverless limits. Done 2026-05-16 on branch `optimize/phase-10-refresh-worker-cron-spike`; added a bounded public refresh planner, a read-only Vercel cron dispatch endpoint, retry/backoff tests, and boundary notes in `docs/phase-10-refresh-worker-cron-spike.md`.
- [x] Local overlay integration: keep local watchlist, account points, purchased works, subscriptions, and annotations as a private overlay applied after public cached results load. Done 2026-05-16 on branch `optimize/phase-10-local-overlay-integration`; added a cloned local overlay applicator, privacy-preserving serialization tests, and integration notes in `docs/phase-10-local-overlay-integration.md`.
- [x] Phase 10 verification: prove that a repeated search can render from cache first, stale cache can refresh safely, private local fields never appear in cloud payloads, and local-only mode still works with cloud cache disabled. Done 2026-05-16 on branch `optimize/phase-10-verification`; added opt-in `preferCache` cache-first search responses with background refresh, local-only fallback coverage, and verification notes in `docs/phase-10-verification.md`.
